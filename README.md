...
然后，当构造函数退出时，运行时返回新创建的实例。所以构造函数没有返回值的原因是因为它不是由代码直接调用，而是由运行时的内存分配和对象初始化代码调用。  
...
- <u>**静态成员变量只能使用静态成员函数来访问和修改.(不是初始化)**</u>
...
- <u>**虚析构函数的好处是: 函数在使用父类指针进行子类的传递的时候,可以在父类函数下直接进行delte , 而且这个delte 会调用子类的,然后再去调用父类的.这样内存就不会泄露**</u>
