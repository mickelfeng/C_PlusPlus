...
- <u>成员函数尾部出现 const ,修饰是 this 指针,就不允许修改对象的任何数据内容.(`const 类名 *cosnt this`)</u>
...
- <u>**符号表有可能在常量区有可能在text段(去找[内存笔记](内存笔记.md));**</u>
...
- <u>类的声明周期结束后会调用析构函数,如果函数中有多个类,那么会实行先入后出原则,最后构造的先析构(出栈)</u>
...
- <u>**做操符会作用在左边的数据上, 如果是对象的话,就回去对象中寻找操作符重载,并调用**</u>
...
- **重载new运算符和delete运算符可以获得用户申请内存空间的大小.**

  - **`void* operator new(size_t nSize){ return malloc(nSize);}`**
    - **`void* operator new(void* pMem){ free(pMem);}`**
  - **`void operator new[](size_t nSize){return malloc(nSize);}`**
...
> - <u>**如果说一个普通类,继承拥有纯虚函数的类,那就必须要重写这个纯虚函数, 如果不重写纯虚函数,那么这个普通类依然是一个抽象类.依然不能被实例化,如果向实例化, 那么必须重写这个纯虚函数.(有几个纯虚函数,就必须重写几个纯虚函数)**</u>
...
> 4. <u>**如果没有在表中找到,那么代表父类和子类的虚函数表中都没有相对应的虚函数,这个时候就从父类中查找(因为指针停留在了父类中) 普通函数成员,如果匹配则输出,如果没有匹配的则回到子类,继续寻找,如果还是没有,那么就报错失败**</u>
...
```c
int a = 10;
if ( a <u> 10){}   // 那怕这么写, 在 a 绝对等于10 的情况下,if也会是静态联编,也会运行进行判断和选择.
